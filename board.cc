...
TODO - Not tested yet.
*/
// Board::Board(): squares{}, textDisplay {make_shared<TextDisplay>()}, gui{new
// GUI} { // later add: gui{new GUI()}
//     for (int i = 1; i <= 8; ++i) {
//         squares.push_back({
...
Board::Board()
    : squares{},
      textDisplay{make_shared<TextDisplay>()} {  // later add: gui{new GUI()}
  for (int i = 1; i <= 8; ++i) {
    squares.push_back({
        Square(Position(A, i), nullptr, textDisplay),
        Square(Position(B, i), nullptr, textDisplay),
...
Parameters: istream& in, bool printBoard (determines whether or not to print
after modification)
--------------------------------------------------------------------------------------------------
  ...
  do {
    ...
    if (arg1 == "+") {  // adding a piece: + K e1
      ...
      if (arg2.size() == 1 &&
          ...
          arg3.size() == 2 && 'A' <= toupper(arg3[0]) &&
          toupper(arg3[0]) <= 'H' && '1' <= arg3[1] &&
          arg3[1] <=
              '8') {  // if condition checking for valid adding piece command

        shared_ptr<ChessPiece> piece;
        switch (arg2[0]) {
          case 'P':
            piece = std::make_shared<Pawn>(true);
            break;
          case 'R':
            piece = std::make_shared<Rook>(true);
            break;
          case 'N':
            piece = std::make_shared<Knight>(true);
            break;
          case 'B':
            piece = std::make_shared<Bishop>(true);
            break;
          case 'Q':
            piece = std::make_shared<Queen>(true);
            break;
          case 'K':
            piece = std::make_shared<King>(true);
            break;

          case 'p':
            piece = std::make_shared<Pawn>(false);
            break;
          case 'r':
            piece = std::make_shared<Rook>(false);
            break;
          case 'n':
            piece = std::make_shared<Knight>(false);
            break;
          case 'b':
            piece = std::make_shared<Bishop>(false);
            break;
          case 'q':
            piece = std::make_shared<Queen>(false);
            break;
          case 'k':
            piece = std::make_shared<King>(false);
            break;
        }
      ...
      in >> arg2;
      if (arg2.size() == 2 && 'A' <= toupper(arg2[0]) &&
          toupper(arg2[0]) <= 'H' && '1' <= arg2[1] && arg2[1] <= '8') {
        if (squares[arg2[1] - '1'][toupper(arg2[0]) - 'A'].getState() !=
            nullptr) {
      ...
      if (arg2 == "white") {
        ...
        cout << "Invalid input, only accept either \"white\" or \"black\"."
             << endl;
      }
    }
      ...
      for (int i = 0; i < 8; i++) {
        if (squares[0][i].getPiece().first == PieceType::Pawn ||
          ...
          std::cout
              << "Invalid setup, pawns cannot be in the first and last rows"
              << endl;
          valid = false;
        }
      }
...
void Board::castling(Move move) {
  ...
  if (kingEndCol == kingStartCol + 2) {
    ...
    // // Move the rook
    // squares[kingRow][kingEndCol -
    // 1].setState(squares[kingRow][7].getState());
    // squares[kingRow][7].setState(emptyState);

    squares[kingRow][7].setState(
        nextPiece);  // we used the swap instead of simply assigning in setState
  ...
  // Perform queen-side castling
    ...
    // // Move the rook
    // squares[kingRow][kingEndCol +
    // 1].setState(squares[kingRow][0].getState());
...
void Board::enPassant(Move move) {
  ...
  // if (squares[endY][endX].getState()->getPiece().second) { // White pawn
  // capturing
  ...
  if (beginPeice.first == PieceType::Pawn &&
      endPiece.first == PieceType::Empty &&
      (startX + 1 == endX || startX - 1 == endX) &&
      (startY + 1 == endY || startY - 1 == endY)) {
...
void Board::promote(Position pos, char c) {
  ...
  switch (c) {
    case 'Q':
      newPiece = make_shared<Queen>(Queen(getPiece(pos).second));
      break;
    case 'N':
      newPiece = make_shared<Knight>(Knight(getPiece(pos).second));
      break;
    case 'R':
      newPiece = make_shared<Rook>(Rook(getPiece(pos).second));
      break;
    case 'B':
      newPiece = make_shared<Bishop>(Bishop(getPiece(pos).second));
      break;
    default:
      break;
  }
...
Move Board::previousMove() {
  ...
  return {{static_cast<xlocation>(1), 0},
          {static_cast<xlocation>(1), 0}};  // the default
}
...
std::vector<PotentialMoves> Board::allPotentialMoves(bool colour) {
  // std::cout << "The colour is " << colour << " in all Potential Moves" <<
  // std::endl;
  vector<PotentialMoves> potentialMoves;
  for (vector<Square>& line : squares) {
    for (Square& square : line) {
      if (square.getPiece().first != PieceType::Empty &&
          square.getPiece().second == colour) {
        potentialMoves.push_back(
            {square.getPosition(), square.getPossibleMoves(this)});
      }
    }
  }
  ...
  //     end: moves.second){ std::cout << end.first << " " << end.second <<
  //     std::endl;
  //     }
  // }
  ...
  for (const PotentialMoves& moves : potentialMoves) {
    if (getPiece(moves.first).second != colour) {
      for (const Position& destination : moves.second) {
        if (getPiece(destination).first == PieceType::King &&
            getPiece(destination).second == colour) {
          return true;
        }
      }
    }
...
void Board::testMove(Move move, bool update) {  // this is mainly used in king
  if (!update) {
    squares[move.first.second - 1][(static_cast<int>(move.first.first)) - 1]
        .minusMoves();
    squares[move.first.second - 1][(static_cast<int>(move.first.first)) - 1]
        .minusMoves();
  }
  squares[move.second.second - 1][(static_cast<int>(move.second.first)) - 1]
      .check();
  nextMove(move, update);
}

void Board::reverseMove(
    Move move, bool update) {  // reverse the move and switch back to actual
                               // piece instead of temp - used in king
  nextMove(move, false);
  if (update) {
    squares[move.first.second - 1][(static_cast<int>(move.first.first)) - 1]
        .minusMoves();
